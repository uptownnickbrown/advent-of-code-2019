...
class ParameterSet:
    def __init__(self, computer, operation):
        ...
        self.modes.extend([0] * (operation.param_count - len(operation.modes)))
        self.params = [Parameter(self.computer, entry, self.modes[i]) for i, entry in enumerate(self.computer.memory[self.computer.pointer + 1:self.computer.pointer + 1 + operation.param_count])]
...
class Parameter:
    def __init__(self, computer, entry, mode):
        ...
        self.mode = mode
        if self.mode == 0:
            self.val = self.computer.memory[self.entry]
        elif self.mode == 1:
            self.val = self.entry
        else:
...
class Computer:
    ...
    def run_step(self):
        ...
        if op.instruction == 1:
            ...
            self.write(out.entry, op_1.val + op_2.val)
            ...
            self.write(out.entry, self.inputs.pop(0))
            self.incr_pointer(op)
            ...
            if check_1.val < check_2.val:
                self.write(out.entry, 1)
            else:
                self.write(out.entry, 0)
            self.incr_pointer(op)
    ...
    def run_to_pause(self, inputs=None):
        self.inputs = inputs
        while self.complete == False:
            if self.run_step() == True:
                break

    def run_program(self, noun=None, verb=None, inputs=None):
        self.inputs = inputs
        self.pointer = 0
        self.outputs = []
        if noun:
